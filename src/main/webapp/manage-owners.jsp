...
<title>Manage Owners | Apartment MS</title>
...
<style>
    ...
    .owner-card p { margin: 0; font-size: 0.9rem; color: #6c757d; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    .owner-card p strong { display: block; color: #495057; font-size: 0.8rem; margin-bottom: 4px; font-weight: 500; }
...
<body>
    <div class="dashboard-wrapper">
        <c:set var="pageName" value="owners" scope="request" />
        ...
        <main class="main-content">
            ...
            <div class="page-container">
                <div class="form-container">
                    <div class="form-card">
                        <h2>Assign Owner to Apartment</h2>
                        <c:if test="${param.assign == 'success'}">
		                    <div class="alert alert-success">Owner assigned successfully!</div>
		                </c:if>
		                 <c:if test="${param.assign == 'error'}">
		                    <div class="alert alert-error">Error assigning owner. Please try again.</div>
		                </c:if>
                            ...
                            <div class="form-group">
                                ...
                                <select id="ownerId" name="ownerId" class="form-control" required>
                                    ...
                                    <c:forEach var="owner" items="${allOwners}">
                                        <option value="${owner.userId}">${owner.firstName} ${owner.lastName}</option>
                                    </c:forEach>
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="apartmentId">Select Unassigned Apartment</label>
                                <select id="apartmentId" name="apartmentId" class="form-control" required>
                                    <option value="">-- Choose an Apartment --</option>
                                    <c:forEach var="apt" items="${unassignedApartments}">
                                        <option value="${apt.apartmentId}">Block: ${apt.blockName}, Flat: ${apt.flatNumber}</option>
                                    </c:forEach>
                                </select>
                            </div>
                            <button type="submit" class="btn btn-primary btn-full">Assign Owner</button>
                        </form>
                    </div>
                </div>
                ...
                <div class="table-container">
                    <div class="table-card">
                        <div class="table-header">
                             <form action="owners" method="get" class="search-bar-container" id="searchForm">
                        ...
                        <div class="owner-list">
                            <c:if test="${empty ownerList}">
                                <div class="empty-list-message"><p>No owners found.</p></div>
                            </c:if>
                            <c:forEach var="owner" items="${ownerList}">
                                <div class="owner-card">
                                    <p><strong>Name</strong> <c:out value="${owner.firstName} ${owner.lastName}" /></p>
                                    <p><strong>Email</strong> <c:out value="${owner.email}" /></p>
                                    <p><strong>Mobile</strong> <c:out value="${owner.mobile}" /></p>
                                </div>
                            </c:forEach>
                        </div>
                        ...
                        <c:if test="${totalPages > 1}">
                            <div class="pagination">
                                <a href="owners?page=${currentPage - 1}&search=${search}" class="btn ${currentPage <= 1 ? 'disabled' : ''}">Previous</a>
                                <c:forEach begin="1" end="${totalPages}" var="i">
                                    <a href="owners?page=${i}&search=${search}" class="btn ${currentPage == i ? 'active' : ''}">${i}</a>
                                </c:forEach>
                                <a href="owners?page=${currentPage + 1}&search=${search}" class="btn ${currentPage >= totalPages ? 'disabled' : ''}">Next</a>
                            </div>
                        </c:if>
                    </div>
                </div>
